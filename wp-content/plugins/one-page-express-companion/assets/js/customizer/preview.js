...
function bindLiveUpdates(root, $) {
    // live update will fallback for slug prefixed themes options

    // $('[data-theme]').each(function () {
    //     var option = $(this).attr('data-theme');
    //     var self = $(this);
    //     liveUpdateAutoSetting(option, function (to) {
    //         self.add($('[data-theme="' + option + '"]')).html(to);
    //     });
    // });
...
function doPreviewDecoration(root, $) {
    ...
    root.CP_Customizer.on(root.CP_Customizer.events.ELEMENT_DECORATED, function (event, $element) {

        //console.log($element);


        $element.find('[hover-fx]').delay(500).find('.overlay, .swap-inner').css({
            display: 'block',
            opacity: 1
        });

        if ($element.is('[hover-fx]')) {
            $element.delay(500).find('.overlay, .swap-inner').css({
                display: 'block',
                opacity: 1
            });
        }
